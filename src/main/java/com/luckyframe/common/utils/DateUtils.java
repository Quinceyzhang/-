...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils
{
	...
	/**
	 * 获取月份列表，若传入年份为今年则返回前推12个月
	 */
	public static List<String> getMonthLst(String year){
		List<String> monthes = new ArrayList();
		if(LocalDate.now().toString().substring(0, 4).equals(year)){
			for(int i=5;i>=0;i--){
				monthes.add(LocalDate.now().minusMonths(i).toString().substring(0,7));
			}
		}else{
			for(int i=1;i<=5;i++){
				if(i<10){
					monthes.add(year+"-0"+i);
				}else{
					monthes.add(year+"-"+i);
				}
			}
