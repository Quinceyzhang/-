...
/*        List<Project> projects=projectService.selectProjectAll(0);
	...
	{		
		if("1".equals(projectCaseParams.getType())){  
			if(projectCaseParams.getParamsValue().length()>0){
				projectCaseParams.setParamsMinValue(projectCaseParams.getParamsValue());
				projectCaseParams.setParamsMaxValue(projectCaseParams.getParamsValue());
			}else{
				return AjaxResult.error("请填写完整参数值");
			}
		}else{
			if(projectCaseParams.getParamsMaxValue().length()>0 && projectCaseParams.getParamsMinValue().length()>0){
	...
	public String edit(@PathVariable("paramsId") Integer paramsId, ModelMap mmap)
	{
		...
		System.out.println(projectCaseParams);
	...
	{		
        try
        {	if(projectCaseParamsService.isOccupy(ids)==0){
        		return toAjax(projectCaseParamsService.deleteProjectCaseParamsByIds(ids));
	        }else{
	        	return AjaxResult.error("该参数已经被仪器占用，不能删除！");
	        }
