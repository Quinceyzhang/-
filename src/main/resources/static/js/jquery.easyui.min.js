...
opts=$.extend({},$.messager.defaults,{content:"<div class=\"messager-icon messager-question\"></div>"+"<div class=\"messager-content\">"+opts.msg+"</div>"+"<br/>"+"<div style=\"clear:both;\"/>"+"<div><input class=\"messager-input\" type=\"text\"/></div>"},opts);
if(!opts.buttons){
opts.buttons=[{text:opts.ok,onClick:function(){
...
var data={};
data[_4c0[1]]=_4bf;
var _4c1=$.ajax({url:_4c0[0],dataType:"json",data:data,async:false,cache:false,type:"post",error:function(jqXHR){doError(jqXHR);}}).responseText;
return _4c1=="true";
...
show(_576);
    ...
    var t = _578(e.target);
    if(t.hasClass("calendar-day")){
        var clickOk=$(_576).parent().siblings(".datebox-button").find("a[datebox-button-index='1']");
...
//TODO 自定义treegrid
if (_930) {
	baseId_1 = _92d+"-"+(_930?1:2)+"-";
} else {
	baseId_2 = _92d+"-"+(_930?1:2)+"-";
}
...
function doError(jqXHR) {
	if(jqXHR.readyState == 0) {
		top.location = localStorage.getItem(CUR_REFERRER);
	}
}
//修复datagrid序号在超过10000的情况下显示不全的问题
//调用：在onLoadSuccess方法中调用：$(this).datagrid("fixRownumber");
