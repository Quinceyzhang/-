...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            open1: function (title, url, width, height, dtName) {
            	//如果是移动端，就使用自适应大小弹窗
            	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            	    width = 'auto';
            	    height = 'auto';
            ...
            // 弹出层全屏
            openViewFull: function (title, url, width, height) {
            	//如果是移动端，就使用自适应大小弹窗
            	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            	    width = 'auto';
            	    height = 'auto';
        ...
        operate: {
         ...
         // 复制信息
            copyProject: function() {
            	...
            	var id = rows.join();
            	$.modal.confirm("确定复制该条" + $.table._option.modalName + "信息吗？", function() {
	            	var url = $.common.isEmpty(id) ? $.table._option.copyProjectUrl : $.table._option.copyProjectUrl.replace("{id}", id);
         ...
         // 添加信息
            add1: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                $.modal.open1("添加" + $.table._option.modalName, url);
            },
            // 修改信息
            edit: function(id) {
            	var url = "/404.html";
            ...
            edit1: function(id) {
            	...
            	$.modal.open1("修改" + $.table._option.modalName, url);
            },
            // 复制信息
            copy: function(id) {
            	var url = "/404.html";
            ...
            customFullPlan: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.customPlanUrl.replace("{id}", id);
            	} else {
            	    var row = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            	    url = $.table._option.customPlanUrl.replace("{id}", row);
            	}
            	$.modal.open1($.table._option.childrenModalPlanName, url);
            },
            customFullEquip: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.customEquipUrl.replace("{id}", id);
            	} else {
            	    var row = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            	    url = $.table._option.customEquipUrl.replace("{id}", row);
            	}
            	$.modal.open1($.table._option.childrenModalEquipName, url);
            },
            customFullDetail: function(id,status) {
            	sessionStorage.setItem('jczxgl', id);
            	sessionStorage.setItem('jczxglzt', status);
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	...
            	$.modal.openFull($.table._option.childrenModalDetailName, url, null, null, "保存");
            },
            // 保存信息
            ...
            saveJsonUnBack1: function(url, data, localUrl) {
            	var config = {
        	        url: url,
        	        type: "post",
        	        dataType: "json",
            ...
            //上传
            upLoad: function(url,data,formData){
            	...
            	var config={
        	        ...
        	        success: function(result) {
        	        	formData += "&postSort=" + result.msg;
        	        	console.log(formData);
        	        	$.operate.save(url + "post/add",formData);
        	        }
            	};
            	$.ajax(config)
            },
            caculateResult: function(url,params){
            	var formdata = {
		    			"projectId":params,
		    	};
